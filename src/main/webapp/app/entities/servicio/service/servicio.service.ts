...
export class ServicioService {
  ...
  protected convertDateFromClient(servicio: IServicio): IServicio {
    return Object.assign({}, servicio, {
      fechaInicio: servicio.fechaInicio?.isValid() ? servicio.fechaInicio.toJSON() : undefined,
      fechaFin: servicio.fechaFin?.isValid() ? servicio.fechaFin.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((servicio: IServicio) => {
        servicio.fechaInicio = servicio.fechaInicio ? dayjs(servicio.fechaInicio) : undefined;
        servicio.fechaFin = servicio.fechaFin ? dayjs(servicio.fechaFin) : undefined;
      });
    }
    return res;
  }
