...
class ProductosResourceIT {
    ...
    @Mock
    private ProductosRepository productosRepositoryMock;

    @Autowired
    private ProductosMapper productosMapper;

    @Mock
    private ProductosService productosServiceMock;
    ...
    void getAllProductosWithEagerRelationshipsIsEnabled() throws Exception {
        when(productosServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restProductosMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(productosServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
